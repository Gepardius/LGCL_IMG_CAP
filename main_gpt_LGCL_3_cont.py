...
class Trainer:
    def __init__(self,model_config,train_config, dls, task_num=None, task_name=None, neg_prompt_pool=None, use_lgcl=None):
...
#         self.sched = get_linear_schedule_with_warmup(self.optim,num_warmup_steps=0,num_training_steps=total_steps)
        ...
        for image, input_ids, labels in prog:
            ...
            with autocast():
                ...
                for input_id in input_ids:
                    ...
                    nouns = extract_nouns(caption_clean)
                    noun_string = ""
                    for noun in nouns:
                        if noun_string == "":
    ...
    def valid_one_epoch(self,epoch):
        ...
        for batch_idx, (image, input_ids, labels) in enumerate(prog):
            ...
            with autocast():
                ...
                for i in range(len(predicted_tokens)):
                    ...
                    bleu_score = sentence_bleu(target_caption, predicted_caption, weights=(1.0, 0.0, 0.0, 0.0), smoothing_function=smoothing_function)
                    bleu_scores_list.append(bleu_score)
                    
                    # Add references and hypotheses to dictionaries
                    references_dict[batch_idx * len(predicted_tokens) + i] = target_caption  # Already tokenized
                    hypotheses_dict[batch_idx * len(predicted_tokens) + i] = [' '.join(predicted_caption)]  # Add as a single string in a list
                    # hypotheses_dict[batch_idx * len(predicted_tokens) + i] = predicted_caption

                average_bleu = sum(bleu_scores_list) / len(bleu_scores_list)
        ...
        # After processing all batches, compute the CIDEr score for the entire epoch
        cider_score, _ = cider_scorer.compute_score(references_dict, hypotheses_dict)
        cider_scores_list.append(cider_score)
        ...
        average_bleus = sum(average_bleus_list) / len(average_bleus_list)
        average_cider = sum(cider_scores_list) / len(cider_scores_list)

        return val_pxp, val_loss, average_accuracy, average_bleus, average_cider
    ...
    def fit(self, task_num, task_name):
        ...
        for epoch in prog:
            ...
            pxp, loss, average_accuracy, average_bleus, average_cider = self.valid_one_epoch(epoch)
            self.clean()
            ...
            if pxp < best_pxp:
                ...
                training_data_list.append(f'{pxp},{loss},{average_accuracy},{average_bleus},{average_cider},{task},{timestamp}\n')
    ...
    def fit_forgetting(self, task):
        ...
        for epoch in range(1):
            ...
            with open('forgetting_output.csv', 'a') as file:
                ...
                # Write the values in a comma-separated format
                file.write(f'{pxp},{loss},{average_accuracy},{average_bleus},{average_cider},{task},{timestamp}\n')
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        if see_1_caption:
            ...
            train_df_task = train_df[train_df['category_name'] == task_list[task_model]]
            val_df_task = val_df[val_df['category_name'] == task_list[task_model]]
            ...
            image = r"C:\Test\LGCL _cap\data\MSCOCO\images_200\COCO_train2014_000000000025.jpg"
            ...
            actual_cap = "a giraffe standing up nearby a tree"

            print(f"{gen_caption}\ntemp: {t}\ndeterministic generation: {det}")
            print(f"{actual_cap}")

            # Tokenize the captions (split by spaces)
            gen_caption_tokens = gen_caption.split()
            actual_caption_tokens = actual_cap.split()

            # Use smoothing function to handle lack of higher-order n-gram matches
            smoothing_function = SmoothingFunction().method1

            # Step 2: Calculate BLEU score
            # Note that the actual caption needs to be a list of lists, as BLEU expects multiple references.
            bleu_score = sentence_bleu([actual_caption_tokens], gen_caption_tokens, weights=(1.0, 0.0, 0.0, 0.0), smoothing_function=smoothing_function)

            # Step 3: Print the BLEU score
            print(f"BLEU Score: {bleu_score:.4f}")

            # gen_caption, tokens, logits, loss = trainer.generate_caption_with_prompt(image,temperature=t,deterministic=det)
            # print(f"{gen_caption}\ntemp: {t}\ndeterministic generation: {det}")
