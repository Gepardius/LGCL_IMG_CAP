...
class Dataset:
    def __init__(self, df, tfms, task_name):
        self.df = df
        self.tfms = tfms
    ...
    def __getitem__(self,idx):
        ...
        # prompt_list = ["This is a picture of", "A picture of", "An image of", "The picture shows", "An image shows", "An image where", "A picture where"]

        # task_prompt = random.choice(prompt_list)

        task_prompt = f"A picture of "
        task_prompt = ""

        caption = f"{task_prompt}{caption}<|endoftext|>"
...
class VisionGPT2Model(nn.Module):
    ...
    def forward(self,image,input_ids,labels=None,text_embedding=None):
...
class Trainer:
    def __init__(self,model_config,train_config, dls, task=None, task_name=None):
...
#         self.sched = get_linear_schedule_with_warmup(self.optim,num_warmup_steps=0,num_training_steps=total_steps)
    ...
    def train_one_epoch(self,epoch,task_name):
        
        prog = tqdm(self.train_dl,total=len(self.train_dl))
        
        running_loss = 0
        
        prompt_pos = f"A picture of {task_name}"
        input_ids = self.tokenizer(prompt_pos, return_tensors="pt").input_ids
        ...
        if task_name == "airplane":
            task_name_neg = "giraffe"
        elif task_name == "clock":
            task_name_neg = "airplane"
        elif task_name == "cat":
                task_name_neg = "clock"
                ...
                task_name_neg = "car"
        elif task_name == "giraffe":
            task_name_neg = "bird"

        prompt_neg = f"A picture of {task_name_neg}"
        ...
        for image, input_ids, labels in prog:
            ...
            with autocast():
                ...
                loss, image_embedding = self.model(image,input_ids,labels,text_embedding)
                ...
                #         noun_string = noun
                #     else:
                #         noun_string = noun_string + " and " + noun
                ...
                # prompt = f"A picture of {noun_string}"
                ...
                # Triplet loss: Encourages the image embedding to be closer to the positive and farther from the negative
                loss_lgcl = 1 - pos_similarity + neg_similarity
                loss_lgcl = loss_lgcl.mean()
                # print(f"loss_lgcl {loss_lgcl}")
    ...
    def valid_one_epoch(self,epoch):
        ...
        for image, input_ids, labels in prog:
            ...
            with autocast():
                ...
                loss, image_emb = self.model(image,input_ids,labels)
                running_loss += loss.item()
    ...
    def fit(self, task, task_name):
        ...
        for epoch in prog:
            ...
            self.train_one_epoch(epoch, task_name)
            self.clean()
...
if __name__ == '__main__':
    ...
    if train_true:
        ...
        for task in task_list:
            ...
            train_ds = Dataset(train_df_task,train_tfms, task_name)
            val_ds = Dataset(val_df_task,valid_tfms, task_name)
            ...
            trainer = Trainer(model_config,train_config,(train_dl,val_dl), task_num, task_name)
    ...
    if forg_calc:
        for task in range(len(task_list)-1, -1, -1):
            ...
            trainer = Trainer(model_config,train_config,(train_dl,val_dl), task_model, task_name)
